...
- (BOOL)application:(UIApplication *)app handleOpenURL:(nonnull NSURL *)url
{
    ...
    BOOL isOurCode  = [query hasPrefix:@"RoomId"];
    
    if (isOurCode)
    {
        NSRange roomRang = [query rangeOfString:@"RoomId="];
        NSString *roomID = [query substringWithRange:NSMakeRange(roomRang.length, 6)];
        
        NSRange nameRang = [query rangeOfString:@"GameName="];
        NSString *name   = [query substringFromIndex:nameRang.location + nameRang.length];
        
        
        NSString *result = [[roomID stringByAppendingString:@"|"] stringByAppendingString:name];
        
        UnitySendMessage("root", "QuickEnterInfo", result.UTF8String);
    }
